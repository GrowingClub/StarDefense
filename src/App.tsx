...
export default function App() {
  ...
  const continueToNextLevel = () => {
    // The GameCanvas will handle the level increment via its internal initGame(false)
    // But we need to tell it to start playing again.
    // Actually, it's better if App manages the level increment to keep state in sync.
    setGameState('PLAYING');
  };
          ...
          <div className="flex flex-col gap-2">
            <div className="bg-black/60 backdrop-blur-md border border-white/10 p-3 rounded-xl flex items-center gap-4">
              <div className="flex flex-col">
                <span className="text-[10px] uppercase tracking-widest text-white/50 font-mono">{t.level}</span>
                <span className="text-2xl font-bold font-mono text-emerald-400">{level}</span>
              </div>
              ...
              <div className="flex flex-col">
                ...
                <span className="text-sm font-mono text-white/70">{level * 200}</span>
              </div>
            </div>
          </div>
              ...
              <div className="mb-6 flex justify-center">
                <div className="w-16 h-16 bg-emerald-500/10 rounded-2xl flex items-center justify-center border border-emerald-500/20">
                  {gameState === 'START' ? <Shield className="text-emerald-400 w-8 h-8" /> : 
                   gameState === 'WON' || gameState === 'LEVEL_UP' ? <Zap className="text-yellow-400 w-8 h-8" /> : 
                   <Target className="text-red-400 w-8 h-8" />}
                </div>
              </div>

              <h1 className="text-4xl font-bold tracking-tight mb-2">
                {gameState === 'START' ? t.title : 
                 gameState === 'LEVEL_UP' ? t.levelUp :
                 gameState === 'WON' ? t.victory : t.gameOver}
              </h1>
              
              <p className="text-zinc-400 mb-8 leading-relaxed">
                {gameState === 'START' ? t.instructions : 
                 gameState === 'LEVEL_UP' ? t.levelSuccess :
                 gameState === 'WON' ? t.winMsg : t.lossMsg}
              </p>

              {gameState !== 'START' && (
                <div className="mb-8 p-4 bg-black/40 rounded-2xl border border-white/5">
                  <span className="text-xs uppercase tracking-widest text-zinc-500 block mb-1">{t.totalScore}</span>
                  <span className="text-4xl font-bold font-mono text-emerald-400">{totalScore}</span>
                </div>
              )}
              ...
              <div className="flex flex-col gap-3">
                {gameState === 'LEVEL_UP' ? (
                  <button 
                    ...
                    className="w-full py-4 bg-yellow-500 hover:bg-yellow-400 text-black font-bold rounded-2xl transition-all active:scale-95 shadow-lg shadow-yellow-500/20"
                  >
                    {t.nextLevel}
                  </button>
                ) : (
